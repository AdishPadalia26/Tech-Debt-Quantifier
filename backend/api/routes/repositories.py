...
async def get_repo_history(repo_url: str, user: User | None = Depends(get_current_user_optional)):
    ...
    db = SessionLocal()
    try:
        history = get_scan_history(db, repo_url, user_id=user_id, limit=10)
        trend = get_debt_trend(db, repo_url, user_id=user_id)
...
async def get_repo_history_rich(repo_url: str, user: User | None = Depends(get_current_user_optional)):
    ...
    db = SessionLocal()
    try:
        trend = get_rich_repo_trend(db, repo_url, user_id=user_id)
...
async def list_repositories(user: User | None = Depends(get_current_user_optional)):
    ...
    db = SessionLocal()
    try:
        repos = get_all_repositories(db, user_id=user_id)
...
async def get_repository_summary(
    repo_url: str, user: User | None = Depends(get_current_user_optional)
):
    ...
    db = SessionLocal()
    try:
        summary = get_repo_summary_rollup(db, repo_url, user_id=user_id)
        if summary is None:
...
async def get_repository_triage(
    ...
    db = SessionLocal()
    try:
        triage = get_repo_triage_stats(db, repo_url, user_id=user_id)
        if triage is None:
            raise HTTPException(404, "No completed scans found for repository")
        return triage
    finally:
...
async def get_repository_changes(
    ...
    db = SessionLocal()
    try:
        changes = get_repo_change_rollup(db, repo_url, user_id=user_id)
        if changes is None:
            raise HTTPException(404, "No completed scans found for repository")
        return {
            "github_url": repo_url,
